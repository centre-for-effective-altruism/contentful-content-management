...
module.exports = function contentfulContentManagement (opts){
    ...
    */
    function queueFactory(items, fn, progressDescription = 'Processing...') {
        ...
        const queue = new PromiseQueue(options.queue_options);
    ...
    */
    ...
    /* 
    *   Create a queue method for multiple items (Assets or Entries)
    *   @func
    *   @name itemQueue
    *   @param command — the command to execute on each item in the itemCollection
    *   @param itemCollection — an Array of Entries/Assets or an EntryCollection/AssetCollection
    */
    const itemQueue = function(command, itemCollection) {
        // we just have an array of Entries/Assets
        if (Array.isArray(itemCollection) && !itemCollection.items) {
            return queueFactory(itemCollection, function(item){
                return item[command]();
            },`<Item.${command}>`)
        }
        // we have an EntryCollection/AssetCollection
        if (typeof itemCollection === 'object' && itemCollection.items){
            return queueFactory(itemCollection.items, function(item){
                return item[command]();
            },`<Item.${command}>`);
        }
